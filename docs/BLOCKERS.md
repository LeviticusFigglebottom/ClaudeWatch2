...
|---|---|---|
| No physical GPU in the build container (Mesa lavapipe only) | Rendering at ~5–10 fps under Xvfb; every screenshot costs 20–40 s | Screenshot harness runs under `xvfb-run` with `--rendering-driver vulkan`; frame time budgets were measured relative to lavapipe and extrapolated, not on target hardware (see `docs/PERFORMANCE.md`) |
